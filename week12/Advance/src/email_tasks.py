...
class EmailTasks:
    ...
    def search_emails(self, mailbox='INBOX', criteria='UNSEEN', from_sender=None, subject_contains=None):
        ...
        try:
            ...
            if from_sender: search_query.extend(['FROM', EmailTasks._imap_quote(from_sender)])
            if subject_contains: search_query.extend(['SUBJECT', EmailTasks._imap_quote(subject_contains)])
